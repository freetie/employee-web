			...
			<div class="card-body">
				<form action="/employee/create" novalidate method="post">
					<input type="hidden" name="id"> <input type="hidden"
						name="accountId">
					<div class="mb-3">
						<label for="job-number" class="form-label">社員番号</label> <input
							type="text" class="form-control" id="job-number" name="jobNumber"
							maxlength="8" pattern="[A-Z][A-Za-z]{0,3}[0-9]{4,7}" required />
					...
					<div class="mb-3">
						<label for="age" class="form-label">年齢</label> <input
						...
						<div>
							<div class="form-check form-check-inline">
								<input class="form-check-input" type="radio" name="gender"
									id="gender-null" value="" checked /> <label
									class="form-check-label" for="gender-null">未選択</label>
							</div>
							<div class="form-check form-check-inline">
								<input class="form-check-input" type="radio" name="gender"
									id="gender-male" value="MALE" /> <label
						...
						<div>
							<div class="form-check form-check-inline">
								<input class="form-check-input" type="radio"
									name="maritalStatus" id="marital-status-null" value="" checked />
								<label class="form-check-label" for="marital-status-null">未選択</label>
							</div>
							<div class="form-check form-check-inline">
								<input class="form-check-input" type="radio"
									name="maritalStatus" id="marital-status-married"
									value="MARRIED" /> <label class="form-check-label"
									for="marital-status-married">既婚</label>
							</div>
							<div class="form-check form-check-inline">
								<input class="form-check-input" type="radio"
									name="maritalStatus" id="marital-status-unmarried"
									value="UNMARRIED" /> <label class="form-check-label"
									for="marital-status-unmarried">未婚</label>
							</div>
						</div>
					</div>
					<div class="mb-3">
						<label class="form-label" for="hire-date">入社日</label> <input
							type="date" class="form-control" id="hire-date" name="hireDate"
							placeholder="入社日" min="1990-01-01" required />
						<div class="invalid-feedback">受け入れられる日付の範囲は1990-01-01から今日の間です。</div>
					</div>
					<div class="mb-3">
						<label class="form-label" for="birth-date">生年月日</label> <input
							type="date" class="form-control" id="birth-date" name="birthDate"
							placeholder="生年月日" min="1990-01-01" max="2010-12-31" />
						<div class="invalid-feedback">受け入れられる日付の範囲は1960-01-01から2010-12-31の間です。</div>
					</div>
					<div class="mb-3">
						<label for="email" class="form-label">Eメール</label> <input
							type="email" class="form-control" id="email" name="email" />
						<div class="invalid-feedback">正しいメールアドレスを入力してください。</div>
					</div>
					<div class="mb-3">
						<label for="phone" class="form-label">携帯番号</label> <input
							type="tel" class="form-control" id="phone"
							pattern="^0[9872]0\d{8}$" name="phone" />
						<div class="invalid-feedback">正しい携帯番号を入力してください。</div>
					</div>
					<div class="mb-3">
						<label for="account-name" class="form-label">アカウント</label> <input
							type="text" class="form-control" id="account-name"
							name="accountName" pattern="^[A-Z][A-Za-z]{7,15}$" required />
						<div class="invalid-feedback" id="account-name-feedback">英語の文字を入力してください。最初の文字は大文字で、長さは8〜16文字です。</div>
					</div>
					<div class="mb-3">
						<label for="account-password" class="form-label">パスワード</label> <input
							type="password" class="form-control" id="account-password"
							name="accountPassword" pattern="^[A-Z]{1}[a-z]{1}\d{6}$" required />
					...
					<div style="text-align: right;">
						...
						<button type="button" class="btn btn-primary"
							id="enter-modify-button">修正</button>
						<button type="button" class="btn btn-secondary"
							id="cancel-modify-button" style="display: none;">キャンセル</button>
      ...
      $(document).ready(() => {
        ...
        if (isNew) {
          ...
          $.get('/employee/detail?id=' + id, (res) => {
            detail = res;
            $jobNumberInput.val(res.jobNumber);
            $nameInput.val(res.name);
            $ageInput.val(res.age);
            $genderRadios.each((index, ele) => {
              $(ele).prop('checked', $(ele).val() === res.gender);
            });
            $maritalStatusRadios.each((index, ele) => {
              $(ele).prop('checked', $(ele).val() === res.maritalStatus);
            });
            if (res.hireYear && res.hireMonth && res.hireDay) {
              $hireDateInput.val(moment(res.hireYear + '-' + res.hireMonth + '-' + res.hireDay).format('YYYY-MM-DD'));
            }
            $birthDateInput.val(res.birthDate);
            $emailInput.val(res.email);
            $phoneInput.val(res.phone);
            $accountNameInput.val(res.accountName);
            $accountPasswordInput.val(res.accountPassword);
          });
